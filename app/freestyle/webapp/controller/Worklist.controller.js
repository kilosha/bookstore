sap.ui.define([
	...
	return BaseController.extend("freestyle.controller.Worklist", {
		...
		onInit : function () {
            ...
            this.setModel(newModel, "newModel");
            this.getModel("newModel").attachRequestSent(() => { console.log("Отправили запрос")})
            newModel.loadData("https://jsonplaceholder.typicode.com/todos/1")
            this.getModel("newModel").attachRequestCompleted(() => { console.log(JSON.stringify(newModel.getData()))})
        ...
        onOpenCreateBookDialogButtonPress: function () {
            ...
            if (!this.pCreateBookDialog) {
                this.pCreateBookDialog = this.loadFragment({
                    name: "freestyle.view.fragments.CreateBookDialog"
                });
            }
        ...
        onPressPlaceAnOrderButton: function () {
            ...
            if (!this.pCreateOrderDialog) {
                this.pCreateOrderDialog = this.loadFragment({
                    name: `freestyle.view.fragments.CreateOrderDialog`
                });
            }
        ...
        handleResponsivePopoverPress: function (oEvent) {
			...
			if (!this._pPopover) {
				this._pPopover = Fragment.load({
					id: oView.getId(),
					name:"freestyle.view.fragments.Popover",
					controller: this
        ...
        onChangeBookInOrderAmountPress: function (oEvent) {
			...
			if (!this._pChangeBookInOrder) {
				this._pChangeBookInOrder = Fragment.load({
					id: oView.getId(),
					name:"freestyle.view.fragments.ChangeBookInOrderAmountPopover",
					controller: this
				}).then(function(oPopover) {
                    oView.addDependent(oPopover);
					return oPopover;
